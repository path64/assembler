  ...
  private:
    ...
    char * ptr_next_ptr() const
    {
      return (ptr_next_size() -
          details::pool::ct_lcm<sizeof(size_type), sizeof(void *)>::value);
    }
...
{ //! No memory in any of our storages; make a new storage,
  ...
  if(!max_size)
    ...
    next_size = std::min BOOST_PREVENT_MACRO_SUBSTITUTION(next_size << 1, max_size*requested_size/ partition_size);
...
{ //! Gets address of a chunk n, allocating new memory if not already available.
  ...
  next_size = std::max BOOST_PREVENT_MACRO_SUBSTITUTION(next_size, num_chunks);
  size_type POD_size = static_cast<size_type>(next_size * partition_size +
      details::pool::ct_lcm<sizeof(size_type), sizeof(void *)>::value + sizeof(size_type));
  char * ptr = (UserAllocator::malloc)(POD_size);
  if (ptr == 0)
...
public:
  ...
  void free BOOST_PREVENT_MACRO_SUBSTITUTION(void *const chunk)
  {
     assert(used_list.count(chunk) == 1);
     assert(free_list.count(chunk) == 0);
